...
@Component({
  ...
  export class TriathlonresultsComponent implements OnInit {
    ...
    public raceResultsCount: number;


    constructor(private api: RaceApiService, private modal: NgbModal) {
  ...
  public addAthlete(race: Race) {
    this.api.getResultsCountByRaceId(+race.raceId).subscribe(data => {
      this.raceResultsCount = data as unknown as number;
    });
    ...
    race.resultsCount++;
    //TODO:on modal I need to make the fields mandatory and then check viewresults on next screen
    modalRef.result.then((result) => {
      ...
      this.api.updateRace(race).subscribe((data: Race) => {
        race = data;
        window.location.reload();
      });
    }, reason => {
